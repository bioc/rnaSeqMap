...
\begin{center}
\textbf{ Vignette for v.1.99.00 - created without XMAP database access.}
\end{center}
...
\code{myslqimport -u mysqlexampleuser -p xmapcorei\_homo\_sapiens\_58 --local seq\_reads }
...
rs <- newSeqReads(17, 7550000,7650000,1) # several genes on the plus strand 
rs <- newSeqReads(17, 7565257,7590856,-1) # tp53
f <- c("test1.bam","test2.bam","test3.bam","test4b.bam","test5b.bam")
...
dd[4800:4810,]    # getting a fragment of the gene
@
...
<<echo=FALSE>>=
nsums <- c(25918,32577,63250,0,126977)
@

<<>>=
nsums
nd.cov <- normalizeBySum(nd.cov, nsums)
RleList2matrix(distribs(nd.cov))[4800:4810,]
@
...
Here, the sample BAM files including data from chromosome 17 and genes ATP1B2, P53, WRAP53, ENFNB3, DNAH2 and RPL29B2 
will be used. 
...
RleList2matrix(distribs(nd.reg))[4800:4810,]
@
...
RleList2matrix(distribs(nd.si))[4800:4810,]
@
\begin{center}
...
RleList2matrix(distribs(nd.fc))[4800:4810,]
@
...
RleList2matrix(distribs(nd.rbcov))[4800:4810,]
@
