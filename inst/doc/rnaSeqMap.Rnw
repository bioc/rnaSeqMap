...
\begin{center}
\textbf{ Vignette for v.0.1.24 - created without XMAP database access! }
\end{center}
...
distribs(nd.cov)[52000:52010,]    # getting a fragment of the gene
@
...
<<echo=FALSE>>=
nsums <- c(1067676,25033137, 85415567,21236253,18152006,25132926)
@

<<>>=
nsums
nd.cov <- normalizeBySum(nd.cov, nsums)
distribs(nd.cov)[52000:52010,]
@
...
<<>>=
apply(distribs(nd.cov),2,max)
apply(distribs(nd.cov),2,mean)
apply(distribs(nd.cov),2,sum)
apply(distribs(nd.cov),2,sd)
@
...
\begin{center}
<<eval=FALSE, fig=TRUE>>=
distrSIPlot(nd.cov, 1, 2, mi=5 ,minsup=5)
@
\end{center}
